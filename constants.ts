...
// Restored default gallery (284 curated Google Drive links)
const DEFAULT_GALLERY_URLS = [
  ...
  'https://docs.google.com/document/d/1zGDy3s-OIMVzMMrJNtv0pY36A20H_gjF6Izq2_Vt3d8/edit?usp=drive_link',
...
export const ARTWORK_ITEMS: MediaItem[] = buildMediaItemsFromUrls(
  DEFAULT_GALLERY_URLS,
  { seed: 'default-gallery', prefix: 'artwork' }
);
...
export const encodeGalleryParam = (
  ...
  // URL-safe encoding keeps "+" and "=" from breaking in query strings
...
export const decodeGalleryParam = (encoded: string | null) => {
  ...
  try {
    // Support both URL-encoded base64 (current) and legacy raw base64 strings
    const normalized = decodeURIComponent(encoded).replace(/ /g, '+');
    const decoded = atob(normalized);
    return JSON.parse(decoded);
  } catch {
    try {
      // Legacy path: the previous implementation encoded the JSON before base64
      const decoded = decodeURIComponent(atob(encoded.replace(/ /g, '+')));
      return JSON.parse(decoded);
    } catch {
      return null;
    }
