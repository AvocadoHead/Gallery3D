...
const App: React.FC = () => {
  ...
  const handleShare = async () => {
    ...
    // keep URL in sync so refresh preserves the state
  ...
  return (
    <div className="w-full h-screen relative bg-gradient-to-br from-[#f8fafc] to-[#e2e8f0] overflow-hidden">
      ...
      <div
        ...
        `}
      >
        <button onClick={() => setBuilderOpen((prev) => !prev)} className="text-left group cursor-pointer">
          ...
          {displayName && <p className="text-[11px] text-slate-500 font-semibold mt-1 ml-[2px]">{displayName}</p>}
        </button>
            ...
            <div className="bg-slate-900 text-white rounded-3xl p-5 flex items-center gap-4 shadow-inner ring-1 ring-white/10">
              ...
              <div className="text-sm leading-tight space-y-1">
                ...
                <div className="flex items-center flex-wrap gap-2 mt-2">
                  ...
                  <a href="https://pay.google.com" target="_blank" rel="noreferrer" className="text-xs underline hover:text-amber-200">
                    Google&nbsp;Pay
                  </a>
                  <a href="https://buymeacoffee.com/Optopia" target="_blank" rel="noreferrer" className="text-xs underline hover:text-amber-200">
                    Buy&nbsp;Me&nbsp;a&nbsp;Coffee
                  </a>
                </div>
              </div>
                ...
                {shareLink && (
                  <div className="flex flex-col gap-2 text-xs text-slate-600">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <a className="px-3 py-1.5 rounded-full bg-blue-600 text-white font-semibold shadow" href={`mailto:?subject=Aether%20gallery&body=${encodeURIComponent(shareLink)}`}>
                        Email
                      </a>
                      <button className="relative px-3 py-1.5 rounded-full bg-slate-200 text-slate-800 font-semibold shadow" onClick={handleCopyLink}>
                        Copy link
                        {toastVisible && (
