...
const NoteForm: React.FC<NoteFormProps> = ({ galleryId, session, onClose }) => {
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    try {
      await postGalleryNote(galleryId, { author_name: name.trim() || undefined, body: body.trim(), allow_share: allowShare }, session);
      setSubmitted(true);
    } catch {
      setError('Could not send note. Please try again.');
    } finally {
      setSending(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/40 backdrop-blur-sm">
      <div className="bg-white w-full sm:max-w-sm sm:rounded-2xl rounded-t-2xl shadow-2xl flex flex-col overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b border-slate-100">
          <h3 className="text-sm font-bold text-slate-900">Leave a note</h3>
          <button
            onClick={onClose}
        ...
        <div className="p-4 space-y-3">
          {submitted ? (
            <div className="text-center py-6 space-y-2">
              ...
              <p className="font-bold text-slate-900 text-sm">Note sent!</p>
            ...
            <>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="What did you think of this gallery?"
                  rows={4}
              ...
              <label className="flex items-start gap-2 cursor-pointer">
                ...
                />
                <span className="text-xs text-slate-500">The gallery owner may share this note publicly</span>
              </label>
