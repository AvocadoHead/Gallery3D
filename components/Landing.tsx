...
const MiniCard = ({ className, label }: { className: string; label: string }) => (
  <div className={`absolute rounded-[1.4rem] bg-white p-1.5 shadow-2xl shadow-black/35 ring-1 ring-white/40 ${className}`}>
    <div className="h-full w-full overflow-hidden rounded-[1rem] bg-gradient-to-br from-slate-100 via-white to-slate-300">
      <div className="h-2/3 bg-[radial-gradient(circle_at_30%_25%,rgba(99,102,241,0.45),transparent_34%),radial-gradient(circle_at_72%_42%,rgba(14,165,233,0.42),transparent_30%),linear-gradient(135deg,#f8fafc,#cbd5e1)]" />
      <div className="flex h-1/3 items-center justify-between px-3">
    ...
    <div className="fixed inset-0 z-[120] flex items-start justify-center overflow-y-auto p-4 sm:items-center sm:p-8 animate-in fade-in duration-700" style={{ touchAction: 'pan-y' }}>
      ...
      <main className="relative grid w-full max-w-6xl items-center gap-8 lg:grid-cols-[1.05fr_0.95fr]">
        <section className="mx-auto w-full max-w-2xl text-center lg:mx-0 lg:text-left">
          ...
          <h1 className="text-balance text-[clamp(2.55rem,7vw,5.9rem)] font-light leading-[0.94] tracking-[-0.065em] text-white">
            Turn a folder of work into a quiet, living gallery.
          </h1>
        ...
        <section className="relative mx-auto hidden aspect-[4/5] w-full max-w-[28rem] lg:block">
          ...
          <MiniCard label="Sphere" className="left-2 top-10 h-48 w-36 rotate-[-9deg]" />
          <MiniCard label="Masonry" className="right-1 top-24 h-56 w-40 rotate-[7deg]" />
          <MiniCard label="Book" className="bottom-14 left-16 h-52 w-44 rotate-[3deg]" />
