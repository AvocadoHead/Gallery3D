...
- **Phase 7 — 3D Book** — new **Book** layout built on **`react-pageflip`** (StPageFlip) rather than a hand-rolled flip, per "use proven code." Paginates `galleryItems` in order at **1 / 2 / 4 items per page** (Look tab), front/back cover, corner-drag + arrow-button navigation, `disableFlipByClick` so item clicks open the lightbox. Arrangement/sizing reuses the Masonry editor (the book follows that order) — no separate in-book editor to keep it un-glitchy. `settings.bookPerPage` persists; `?layout=book` shares.
