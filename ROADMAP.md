...
- **Phase 6 follow-ups** — the separate "Canvas" layout was merged into **Masonry**: Masonry is now the editable layout (grid reflow default + free/overlap sub-mode), edited via **Look → Edit Layout**. Save now persists the full arrangement (canvas x/y/w/h/z/rotation, text blocks, order) — the bug was the save path rebuilding items from the URL textarea; `mergeMediaItems` is now order/canvas-preserving and save uses `galleryItems` as the source of truth. No DB change (items is jsonb).
- **Phase 7 — 3D Book** — ported from Eyal's own proven prototype (`AvocadoHead/the-3d-book`): a real WebGL **skinned-mesh flip book** (r3f + three + drei + `maath`), not a flat CSS flip. `components/book/Book.tsx` keeps the bone-flip math verbatim (jotai atoms → props). `components/book/bookTextures.ts` ports the texture pipeline: Drive links → `lh3.googleusercontent.com` for CORS, `corsproxy.io` fallback, and canvas-composited **1 / 2 / 4 items per page** → dataURL page textures. Click a page (or the arrows) to turn; OrbitControls to look around; title cover page. Arrangement/order reuses the Masonry editor. `settings.bookPerPage` persists; `?layout=book` shares. (Replaced the earlier react-pageflip attempt.)
